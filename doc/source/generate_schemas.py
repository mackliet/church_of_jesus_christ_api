...
schema_example = convert_JSON_to_simple_example(api.user_details, lambda leaf: type(leaf).__name__)

print(json.dumps(actual_example, indent=2, sort_keys=True))
open(f'JSON_schemas/user_details-schema.md', 'w').write('\n'.join(line.replace('"', '') for line in json.dumps(schema_example, indent=2, sort_keys=True).split('\n')))
...
for attr in sorted(dir(api)):
    if re.search('^get_', attr):
        ...
        try:
            ...
            schema_example = convert_JSON_to_simple_example(ret_val, lambda leaf: type(leaf).__name__)
